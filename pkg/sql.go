...
func (r reflectFunc) DoSQL(row map[string]interface{}, funcArgs map[string]interface{}) (ROWS, error) {
	...
	if err != nil {
		log.Printf("detectSQL:[%s] execute failed, err:%s", detectSQL, err.Error())
		return nil, err
	}
	defer detect_rows.Close()
	err = detect_rows.Err()
	if err != nil {
		log.Printf("detect Err:%s", err.Error())
		return nil, err
	}
	...
	if err != nil {
		log.Printf("querySQL:[%s] execute failed, err:%s", querySQL, err)
		return nil, err
	}
	defer esRows.Close()
	...
	if err != nil {
		log.Printf("get columns failed, err:%s", err)
		return nil, err
	}
...
func (r reflectFunc) NestedArray(row map[string]interface{}, funcArgs map[string]interface{}) (ROWS, error) {
	...
	if sqlField == "" || common == "" || pos2Fields == nil || len(pos2Fields) == 0 || fieldsSeprator == "" || groupSeprator == "" {
		log.Printf("Params invalid, sqlField:%+v common:%+v pos2Fields:%+v fieldsSeprator:%+v groupSeprator:%+v row:%+v", sqlField, common, pos2Fields, fieldsSeprator, groupSeprator, row)
	...
	for esName, sqlPos := range pos2Fields {
		if esName == "" || sqlPos == nil || uint64(sqlPos.(float64)) == 0 {
			log.Printf("pos2Fields invalid, pos2Fields:%+v", pos2Fields)
			return nil, errors.New("pos2Fields invalid")
		}
...
func (r reflectFunc) SetDocID(row map[string]interface{}, funcArgs map[string]interface{}) (ROWS, error) {
	...
	if docID == "" || docID == "_id" || row[docID] == nil || row[docID] == "" || row["_id"] != nil {
		log.Printf("docID invalid, docID:%+v row:%+v", docID, row)
