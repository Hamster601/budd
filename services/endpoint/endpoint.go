...
func convertColumnData(value interface{}, col *schema.TableColumn, rule *config.Details) interface{} {
	if value == nil {
		return nil
	}
...
func encodeValue(rule *config.Details, kv map[string]interface{}) string {
	...
	case config.ValEncoderKVCommas:
		var ls []string
		for k, v := range kv {
...
func rowMap(req *model.RowRequest, rule *config.Details, primitive bool) map[string]interface{} {
...
func oldRowMap(req *model.RowRequest, rule *config.Details, primitive bool) map[string]interface{} {
	kv := make(map[string]interface{}, len(rule.PaddingMap))
...
func primaryKey(re *model.RowRequest, rule *config.Details) interface{} {
...
func buildPropertiesByRule(rule *config.Details) map[string]interface{} {
...
func buildPropertiesByMappings(rule *config.Details) map[string]interface{} {
	properties := make(map[string]interface{})
	for _, mapping := range rule.EsMappings {
		property := make(map[string]interface{})
		property["type"] = mapping.Type
